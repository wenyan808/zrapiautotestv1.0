# test_AccountConClient_DerivativeList    衍生品交易风险披露查询    /api/con_client/v1/derivative_list
"""
@File  ：test_AccountConClient_DerivativeList.py
@Author: yishouquan
@Time  : 2020/8/11
@Desc  :  衍生品交易风险披露查询
"""
...
@allure.feature('衍生品交易风险披露查询')
class TestAccountConClientDerivativeList():
    @classmethod
    def setup_class(cls) -> None:
        cls.session = Requests().get_session()
        # login()  # 调用登录接口通过token传出来
        cls.url = console_HTTP + "/api/con_client/v1/derivative_list"
    ...
    @pytest.mark.parametrize('info', get_json(BASE_DIR +
                                              r"/TestData/AccountConsoledata/test_AccountConClient_DerivativeList.json"))
    def test_AccountConClient_DerivativeList(self, info):
        ...
        confirmStatus = info.get("confirmStatus")  # 最近确认状态 是否
        ...
        r = Requests(self.session).post(
            url=self.url, headers=headers, data=payload, title="衍生品交易风险披露查询"
        )
        j = r.json()
        # print(j)
