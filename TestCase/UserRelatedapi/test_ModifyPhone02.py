...
class TestModifyPhone02():
    ...
    def setup_class(cls) -> None:
        ...
        list1 = list(get_registerno())
        paylonewname = [{"phone": list1[0]}]
    ...
    def test_ModifyPhone02(self, oldphone):
        ...
        r = Requests(self.session).post(
            url=url1, headers=headers1, data=payload0, title="修改手机号-当前使用手机号验证"
        )
        ...
        url2 = HTTP + "/as_user/api/user_account/v1/modify_phone_v2"
        paylo = {
        ...
        r1 = Requests(self.session).post(
            url=url2, headers=headers1, data=payload02, title="修改手机号-新手机号验证"
        )
        j1 = r1.json()
