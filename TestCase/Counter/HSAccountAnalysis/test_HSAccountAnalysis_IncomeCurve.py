# test_HSAccountAnalysis_IncomeCurve       收益曲线    /as_trade/api/analysis/v1/income_curve
import json
...
class TestHSAccountAnalysisIncomeCurve():
    ...
    @pytest.mark.parametrize('info',
                             get_json(BASE_DIR + r"/TestData/HSAccountAnalysis/test_HSAccountAnalysis_IncomeCurve.json"))
    def test_HSAccountAnalysis_IncomeCurve(self, info):
        http = list(AccountAuth())[-1]
        url = http + "/as_trade/api/analysis/v1/income_curve"
        # 拼装参数
        ...
        r_info = Requests(self.session).post(
            url=url, headers=headers, data=payload2, title="收益曲线"
        )
