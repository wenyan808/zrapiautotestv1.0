# test_AccountConsole_SelectOpenAgreement     查询开户文件          /api/con_open/v1/select_open_agreement
import json
...
from TestAssertions.AccountConsoleSchemaData.SelectLiveRecordSchema import SelectLiveRecordSchema

from glo import console_JSON, console_HTTP, loginAccount_phone


# @pytest.mark.skip(reason="开发中")
@allure.feature('开户console_查询开户文件')
class TestAccountConsoleSelectOpenAgreement():
    @classmethod
    ...
    def test_AccountConsole_SelectOpenAgreement(self):
        url = console_HTTP + "/api/con_open/v1/select_open_agreement"
        ...
        r = Requests(self.session).post(
            url=url, headers=headers, data=payload, title="查询开户文件"
        )
        ...
        try:
            ...
            #         assert "liveId" in j.get("data")[i]
            #         assert "userId" in j.get("data")[i]
            #         assert "openId" in j.get("data")[i]
            #         assert "videoUrl" in j.get("data")[i]
            ...
            #     jsonschema_assert(j.get("code"), j.get("msg"), j, SelectLiveRecordSchema)
