# test_RStrategy_UnSubscribe        取消订阅策略      /as_trade/api/strategy/v1/un_subscribe
import json
...
@allure.feature('柜台app_取消订阅策略')
class TestRStrategyUnSubscribe():
    @classmethod
    ...
    # @pytest.mark.skip(reason="调试中 ")
    def test_RStrategy_UnSubscribe(self):
        ...
        k_list = r_list.json()
        # print(k_list)
        combinationNo = k_list.get("data")[1].get("combinationNo")
        paylo = {
        ...
        r = Requests(self.session).post(
            url=url2, headers=headers1, data=payload3, title="取消订阅策略"
        )
