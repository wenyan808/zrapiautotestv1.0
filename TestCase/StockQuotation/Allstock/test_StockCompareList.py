# test_StockCompareList     股票对比查询     /as_market/api/stock/compare/v1/list
import json
...
@allure.feature('股票&行业对比_股票对比查询')
class TestStockCompareLineList():
    @classmethod
    def setup_class(cls) -> None:
        cls.session = Requests().get_session()
        cls.url = http + "/as_market/api/stock/compare/v1/list"
    ...
    @pytest.mark.parametrize('info',
                             get_json(BASE_DIR + r"/TestData/AllStockData/test_StockCompareList.json"))
    def test_StockCompareLineList(self, info):
        ...
        r = Requests(self.session).post(
            url=self.url, headers=headers, json=payload1, title="股票对比查询"
        )
