# test_HSAccountAnalysis_profit      盈亏分析     /as_trade/api/analysis/v1/profit
import json
...
@allure.feature('柜台app_盈亏分析')
class TestHSAccountAnalysisProfit():
    @classmethod
    ...
    @pytest.mark.parametrize('info',
                             get_json(BASE_DIR + r"/TestData/HSAccountAnalysis/test_HSAccountAnalysis_profit.json"))
    def test_HSAccountAnalysis_profit(self, info):
        http = list(AccountAuth())[-1]
        url = http + "/as_trade/api/analysis/v1/profit"
        # 拼装参数
        ...
        r_info = Requests(self.session).post(
            url=url, headers=headers, data=payload2, title="盈亏分析"
        )
        ...
        assert j.get("msg") == 'ok'
        # assert j.get("data").get("totalIncome") == 0
        # assert j.get("data").get("incomeRate") == 0
        # assert j.get("data").get("maxRetracementRate")  == 0
        # assert j.get("data").get("winRate")==0
        # assert j.get("data").get("startDate")== startDate
        # assert j.get("data").get("endDate")
        # assert j.get("data").get("everydayUpdateTime")
